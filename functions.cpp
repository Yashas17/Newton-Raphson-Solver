...
void polyDiff(const std::vector<std::vector<double>> &eq,
  ...
  /*
>   The function takes in two 2D vectors: eq and eq1, representing original and
differentiated equations respectively.
>   It differentiates eq and stores it in
eq1.
*/
  eq1 = eq;
...
>   The function calls function polyDiff() to get differentiated equation of
input equation and function f() to evalute the value of equation at given point.
*/
  std::vector<std::vector<double>> eq1;
  polyDiff(eq, eq1);
...
bool polySolve_test() {
  /*
>   We test the working of the polySolve() function by solving an equation with
known analytical solution and comparing the numerical solution of polySolve()
with it.
>   We solve the equation: x^2-x-1 using polySolve().
>   It's positive
root is a well known number known as 'Golden Ratio':(1+sqrt(5))/2 which is
approximately equal to 1.61803. The negative root is approximately equal to
-0.61803.
>   We set the guess solution to 1.6, which is closer to the positive
root to ensure the solution converges to positive root.
>   We use a tolerance
of 1e-3 for the solver and also for comparing the analytical and numerical
solution.
*/
  int nt = 3;
  double x0 = 1.6;
